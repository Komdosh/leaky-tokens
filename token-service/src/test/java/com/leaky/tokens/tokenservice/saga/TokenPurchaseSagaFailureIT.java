...
@SpringBootTest(
    ...
    properties = {
        ...
        "token.saga.simulate-failure=true"
    }
)
@ActiveProfiles("test")
class TokenPurchaseSagaFailureIT extends TokenServiceIntegrationTestBase {
    @Autowired
    ...
    @Test
    void emitsCompensationWithoutAllocatingQuota() {
        ...
        boolean poolExists = tokenPoolRepository.findByUserIdAndProvider(
            UUID.fromString(request.getUserId()), request.getProvider()
        ).isPresent();
        assertThat(poolExists).isFalse();
    }
}
